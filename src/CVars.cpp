...
void CVarImpl::ImGuiDisplayCVars()
{
	...
	for(int i = 0; i < floatCVars.lastCvarIndex; i++)
	{
		...
		ImGui::SliderFloat(param->name.c_str(), &floatCVars.cvars[i].current, floatCVars.cvars[i].initial - floatCVars.cvars[i].initial * 2.0, 2.0 * floatCVars.cvars[i].initial);
	}
